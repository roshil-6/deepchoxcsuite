...
export type HealthWidgetProps = {
    ...
    financial: number;
    execution: number;
    /** When true, skip animated scores — the venture record is still mostly empty. */
    needsSetup?: boolean;
};
        ...
        >
            ...
            <p className="mt-1 text-xs text-[var(--muted)] opacity-80">
                {needsSetup
                    ...
                    : 'How much founder context is on the record — not a grade of strategic brilliance'}
            </p>
            {needsSetup ? (
                <p className="mt-3 text-xs leading-relaxed text-[var(--muted)]">
                    Bars stay hidden until you&apos;ve added enough context (strategy narrative plus product, market, or
                    finance notes). Open Dexo or the desks — scores based only on templates aren&apos;t shown so nothing
                    feels invented.
                </p>
            ) : (
