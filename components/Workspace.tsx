...
export function Workspace() {
  ...
  if (activeRoom === 'dashboard') {
    return (
      <div key="dashboard" className="page-transition flex h-screen flex-1 overflow-hidden bg-[#111113]">
        <Dashboard />
      </div>
    );
  }
  ...
  if (activeRoom === 'calendar') {
    return (
      <div key="calendar" className="flex-1 h-screen overflow-hidden bg-[#111113] page-transition">
        <CalendarView />
      </div>
    );
  }
  ...
  if (activeRoom === 'org_structure') {
    return (
      <div key="org_structure" className="flex-1 h-screen overflow-hidden bg-[#111113] page-transition">
        <OrgStructureBuilder />
      </div>
    );
  }
  ...
  if (activeRoom === 'intelligence_diary') {
    return (
      <div key="intelligence_diary" className="flex-1 h-screen overflow-hidden bg-[#111113] page-transition">
        <IntelligenceDiary />
      </div>
    );
  }
  ...
  if (!activeProject) {
    return (
      <div key="dashboard-fallback" className="flex-1 h-screen overflow-hidden bg-[#111113] page-transition">
        <Dashboard />
      </div>
    );
  }
    ...
    <div key={activeProject?.id || 'workspace'} className="flex-1 flex flex-col h-screen overflow-hidden bg-black relative page-transition">
      ...
      <div className="flex-1 p-6 flex gap-6 overflow-hidden z-0">
        ...
        <div className="flex-[5] flex flex-col min-w-0 h-full relative">
          ...
          <div className="absolute top-4 right-6 z-20 flex bg-[#111113] rounded-full p-1 shadow-2xl border border-red-800/20 transition-all hover:scale-105">
            <button
